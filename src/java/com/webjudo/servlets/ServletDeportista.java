...
public class ServletDeportista extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("Inicio".equals(accion)) {
            ...
            DeportistaDAO deportistaDAO = new DeportistaDAO(new Conexion("dbapolijudo", "polijic", "jdbc:oracle:thin:@localhost:1521:XE"));
            String id = request.getParameter("buscar_dep");
            Deportista deportista;
            try {
                deportista = deportistaDAO.buscarDeportista(Integer.parseInt(id));
                request.setAttribute("mensaje", "El deportista fue encontrado");
                request.setAttribute("nro_identificacion", deportista.getNro_identificacion());
                request.setAttribute("nombres", deportista.getNombre());
                request.setAttribute("apellido1", deportista.getPrimer_apellido());
                request.setAttribute("apellido2", deportista.getSegundo_apellido());
                request.setAttribute("programa_academico", deportista.getPrograma_academico());
                request.setAttribute("semestre", deportista.getSemestre());
                request.setAttribute("grupo", deportista.getGrupo());
                request.setAttribute("medallas", deportista.getMedallas());
                request.setAttribute("direccion", deportista.getDireccion());
                request.setAttribute("telefono", deportista.getTelefono());
                request.setAttribute("celular", deportista.getCelular());
                request.setAttribute("cinturon_actual", deportista.getCinturon_actual());
                request.setAttribute("cinturon_proximo", deportista.getCinturon_proximo());
                request.setAttribute("fecha_nac", deportista.getFecha_nacimiento());
                request.getRequestDispatcher("GestionarDeportista.jsp").forward(request, response);
            } catch (Exception e) {
                Logger.getLogger(ServletDeportista.class.getName()).log(Level.SEVERE, null, e);
                request.setAttribute("mensaje", "No se pudo encontrar el deportista");
                request.getRequestDispatcher("GestionarDeportista.jsp").forward(request, response);
            }
            ...
            try {
                deportistaDAO.modificarDeportista(new Deportista(Integer.parseInt(nro_identificacion), nombres, apellido2, apellido2, fecha_nacimiento, programa_academico, Integer.parseInt(semestre),
                        Integer.parseInt(grupo), medallas, Integer.parseInt(cinturon_actual), Integer.parseInt(cinturon_proximo), direccion, Integer.parseInt(telefono), Integer.parseInt(celular)));
            } catch (Exception e) {
                Logger.getLogger(ServletDeportista.class.getName()).log(Level.SEVERE, null, e);
                request.setAttribute("mensaje", e.getMessage());
